...
class ReportGenerator:
    ...
    def generate_html_report(
        ...
        historical_tickets: Optional[List[Dict[str, Any]]] = None,
        ...
        Args:
            ...
            tickets: Raw ticket data for creation/completion charts (filtered by created date)
            historical_tickets: Ticket data for status tracking charts (filtered by updated date)
    ...
    def _build_html_structure(
        ...
        <div class="section">
            ...
            # Generate in progress tracking chart using historical_tickets
            # Use historical_tickets (updated date) for accurate status tracking
            tickets_for_status = historical_tickets if historical_tickets else tickets
            in_progress_chart = InProgressTrackingChart(tickets_for_status, self.jira_url)
        ...
        <div class="section">
            ...
            # Generate status category distribution chart using historical_tickets
            # Use historical_tickets (updated date) for accurate status tracking
            status_cat_chart = StatusCategoryChart(tickets_for_status, self.jira_url)
